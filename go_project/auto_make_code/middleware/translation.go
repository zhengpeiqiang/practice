	...
	return func(c *gin.Context) {
		...
		default:
			...
			//自定义验证方法
			{
				...
				//验证手机号码
				val.RegisterValidation("valid_phone", func(fl validator.FieldLevel) bool {
					matched, _ := regexp.Match(`^1[3456789]\d{9}$`, []byte(fl.Field().String()))
					return matched
				})
			...
			//自定义验证器
			{
				...
				//手机号码
				val.RegisterTranslation("valid_phone", trans, func(ut ut.Translator) error {
					return ut.Add("valid_mobile", "{0} 格式错误", true)
